...
const uploadProductWithSourceCode = (req, res, next) => {
  // First handle images
  uploadProductImages(req, res, (err) => {
    ...
    // Then handle source code
    uploadSourceCode(req, res, (err) => {
      if (err) {
        return res.status(400).json({
          success: false,
          message: `Source code upload error: ${err.message}`
        });
      }
      
      // Restructure req.files for consistency
      if (!req.files) {
        req.files = {};
      }
      
      // Move uploaded images to req.files.images
      if (req.files && Array.isArray(req.files)) {
        req.files = { images: req.files };
      }
      
      // Add source code file to req.files.sourceCode
      if (req.file) {
        req.files.sourceCode = [req.file];
        delete req.file;
      }
